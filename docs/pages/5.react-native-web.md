...
Make sure that you have followed the getting started guide to and have `react-native-paper` installed and configured before following this guide.
...
First, we have to tell webpack to use `react-native-web` instead of `react-native`. To do that, add the following alias in your webpack config under `resolver`:
...
Now we need tweak this section to load the Material icons from the `react-native-vector` library:
